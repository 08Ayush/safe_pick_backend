...
# Load environment variables from .env file
load_dotenv()
# Use GOOGLE_API_KEY from your .env file
api_key = os.getenv("GOOGLE_API_KEY")

# Check if the API key is found
if not api_key:
    print("ERROR: GOOGLE_API_KEY not found in .env file!")
    print("Please ensure your .env file contains 'GOOGLE_API_KEY=\"YOUR_API_KEY_HERE\"'.")
    exit()
...
def analyze_label(image_path, user_allergies=[]):
    """
    Analyzes a food product label image using Google Gemini to detect allergens.
    """
    base64_image = encode_image(image_path)
    allergy_info = ", ".join(user_allergies) if user_allergies else "none provided"
...
"""
    ...
    try:
        # Set up the model. gemini-1.s5-flash is fast and excellent for vision.
        ...
        # Make the API call using the Google client structure
        response = model.generate_content(
            [prompt, image_part], # Pass both text and image parts
            stream=True,
            # Enforce JSON output for better reliability
            generation_config={"response_mime_type": "application/json"}
        )

        response_text = ""
        # The streaming loop is simpler with the Google library
        ...
        error_response = {
            ...
            "detected_allergens": [],
            "risk_level": "Unknown",
            "recommendation": f"Failed to get a response from the AI model. Error: {e}"
        }
        return json.dumps(error_response)

    # Your robust JSON parsing logic is excellent and will work perfectly.
    json_string_to_parse = response_text.strip()
    ...
    try:
        ...
        # If parsing fails, construct a structured error response.
        print(f"Warning: AI response was not valid JSON. Error: {e}. Raw output: {response_text.strip()}")
        error_response = {
            "extracted_ingredients": [],
            "detected_allergens": [],
...
if __name__ == "__main__":
    ...
    test_allergies = ["milk", "peanuts", "soy"]
    print(f"Analyzing image: {image_path} for allergies: {test_allergies}")
    result = analyze_label(image_path, test_allergies)
    print("\n--- Analysis Result (Raw JSON String) ---")
    print(result)
